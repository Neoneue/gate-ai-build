...
| `AskAiPlaceholderThread` (`ask-ai-placeholder-thread.tsx`) | `AskAiMessage` parts | Design-sourced placeholder copy transcribed verbatim from Figma `1125:4374` — one user turn, one agent reply, authored as plain markdown-shaped HTML. Not wired to data; replace wholesale when the live agent lands. Added 2026-07-27. |
| `ScrollToLatestFab` + `ScrollBottomSentinel` (`ask-ai-scroll-to-latest.tsx`) | custom button + marker div | Jump-to-bottom control for the Ask AI thread (Figma `1149:10955` light / `1125:4280` dark). 32px circle, 16px `ArrowDown`, `bg-control-raised` + `border-border` + `shadow-(--shadow-card-soft)`; absolutely positioned against the message region's wrapper so the panel's `px-4` / `gap-4` supply Figma's 16px right + 16px above-composer offsets with no hard-coded position and no layout shift. Visibility = `!isAtBottom && hasOverflow`, where `isAtBottom` comes from an IntersectionObserver on a zero-height sentinel (60px `rootMargin`) and `hasOverflow` from a ResizeObserver — no scroll listener. Smooth scroll cancels on wheel/touch/key input. `overflow-anchor: none` on the content + `auto` on the sentinel gives native stick-to-bottom during streaming (CSS only). Added 2026-07-27. |

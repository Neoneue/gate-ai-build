...
Three `<article>` cards in fixed ladder order (Free, Pro, Enterprise), laid out `grid @4xl:grid-cols-3 grid-cols-1 grid-rows-[auto_auto_auto_1fr_auto] gap-4`, each card `row-span-5 grid grid-rows-subgrid` plus the Card / surface tier's `border` and `shadow-xs` (design.md §5.1), named here rather than inherited because a subgridding card cannot be a `<Card>`. Five shared slots: header, price, `Separator`, body, caption; only the body is `1fr`, and the card carries exactly ONE rule, above the benefits label. The CTA buttons are deliberately NOT a shared row: they sit inside the body, bottom-anchored with `mt-auto`, so each card's action band is sized by its own buttons. That puts the last button of all three cards on one baseline with no dead space under a single-control card; `pt-6` on the band compounds with the body's `gap-4` for a 40px minimum off the list. Controls use the Button primitive's LIFT family rather than `outline` / `ghost`, because those hover to an opaque `bg-muted` that would cover a tinted card: transparent fill, `hover:bg-lift-8` (design.md §2 "Lift ramp"), edge and ink from `--border` or the card's tier family. The focal card's button takes that card's tier (`lift-pro` / `lift-enterprise`), every plain card's is neutral `lift`, and "Book a demo" is `lift-ghost`; the mapping is one `liftVariant()` on the page, since it depends on the card rather than the action. No mount animation. Below `@4xl` (896px inline-size; 288px per card at the threshold) it is one column, never two: a two-up step would orphan the third rung.
...
**Fill rule:** the ONLY filled control on the page is "Upgrade to Pro" (`promo`), so only the Free-org and Default views carry one; the Pro-org and Enterprise-org views have none, their focal card being marked by tint plus badge instead. The contact primary is `outline` on every view and "Book a demo" is `ghost` directly beneath it, the one fenced ghost on the page (see `PlanAction.variant`: ghost only for a paired secondary with a bordered sibling above it). Outline controls carry a call-site `bg-transparent` so they read as true outlines over a tinted card.
...
**The org's own rung** carries an `outline` "Go to Overview" link (`Button render={<Link to={withTierOf(pathname, "/overview")}>} nativeButton={false}`, label only) on every view: Free card on the Free-org view, Pro card on the Pro-org view, Enterprise card on the Enterprise-org view. That card has no plan action to offer, so the slot sends the user into the product rather than repeating the page's own BackLink; there is no dead control and no back-pointing arrow on the page.

**Enterprise org:** nothing is featured and no slot is empty. The Free and Pro rungs each take an `outline` "Contact support", opening the contact dialog, which is the PRD's "Support routing in place of self-serve upgrade"; the Enterprise rung takes the "Go to Overview" link. There are no disabled buttons on the page, and no `ghost` on this view (the only ghost in the ladder is "Book a demo", which this view does not show).

Plan data lives in `src/data/plans.ts` (`plansFor(tier)`), moved verbatim from the two dialogs; the per-org label differences they carried ("Included in your Free plan:" vs "Included with the Free plan:") are preserved as per-tier variants. Enterprise is the new rung: a bare "Custom" price with no unit (the card renders a `priceSuffix` only when the plan names one), four features (org and team forced settings, private cloud deployment, custom retention, procurement support), actions "Contact us" (`outline`, the SALES path, because a Free or Pro org looking at Enterprise is a prospect) + "Book a demo" (`ghost`, the paired secondary) on the Free and Pro views. The Enterprise-org view's Free and Pro rungs say "Contact support" instead: that org is an existing customer routing to Support. Caption "Billed per seat, changes go through Support." on those two views; the Enterprise-org view reads "Plan changes go through Support.", dropping the seat half because its Billing page already shows the seat charge. On that view the FREE rung's caption also becomes "Available through Support." (shared `SUPPORT_ROUTE_CAPTION`), so both downgrade paths read alike; the other three views keep "Free to use, forever".

Benefits rows sit `gap-4` apart with the title and its detail line `gap-1` apart, so each pair groups four times more tightly than it separates. Only the PROMOTED button wears a glyph (the SparklesIcon on "Upgrade to Pro"), at most one per view: the cards already carry a check per feature row, so a glyph on every control read as decoration. Every row renders ONE shared glyph, lucide `CircleCheck` (`size-4 mt-1 shrink-0`, stroke 1.75, `aria-hidden`), tinted `text-tier-pro` on the featured Pro card and `text-muted-foreground` elsewhere. `PlanFeature` is `{ title, detail }`; the per-feature icons the dialogs carried are gone.

The contact and Book a demo actions open ONE dialog, described below. It was briefly a nested page (2026-09-22) and was reverted the same day: the flow is a short form, not a detail surface.

**Contact / Book a demo dialog.** One `Dialog` serves both actions and carries the LABEL of the button that opened it; that label is `contactFlowTitle(tier, flow)` from `src/data/plans.ts`, the same function that builds the button, so heading and control say the same string by construction. Escape closes and `finalFocus` returns focus to the exact opener. Width caps at `!max-w-[560px]` with `w-[calc(100%-2rem)]` for the phone gutters. Four states off the one-way `?form=` preview param (see §7): `ready`, `loading` (`Skeleton`), `error` (the `OctagonAlert` inline pattern with `role="alert"` plus a no-op "Try again"), `done` (a `role="status"` confirmation). `aria-busy` sits on whichever region wraps the content.

**Two bodies, and the border is the difference.** The CONTACT body is OUR form sitting directly on the dialog surface, no inner frame: on a signed-in surface the guidance is to build the form in our own framework and submit to HubSpot's Forms Submission API, so it wears our tokens and our submit button. Four fields, Name, Work email, Company, Notes, all prefilled from `signedInMember()` and `WORKSPACE_NAME` in `src/data/team-members.ts` except Notes, and all four stay editable. Work email is the only field HubSpot requires by default and is what links a later booking to the same contact record; Company routes the enquiry; Notes captures the intent the ticket says we cannot capture today. Per-field validation fires on blur and on submit, never per keystroke: the invalid control takes `aria-invalid` plus `aria-describedby`, its `Field` takes `data-invalid`, and the message renders inline under its own field through `FieldError`, the pattern `BillingFree.tsx` already uses. The three required fields are marked with the word "Required" in the label row, never colour alone. Submit reads "Submit form".

The DEMO body is a bordered `rounded-md` region reserving 480px, because HubSpot Meetings has no API equivalent and is an isolated iframe we cannot style. **That 480px is a placeholder pending the live widget, not a measurement**: no scheduling link exists yet, the real height is not controllable by the host and has never been measured, and the developer should size the container against the live scheduling page. It is a `min-h`, never a fixed height. The dialog itself caps at `max-h-[90vh]` on an `auto 1fr auto` row template with the body as the scrolling middle row, so an embed taller or shorter than the reserve cannot break the modal. Its own booking form always collects first name, last name and email, HubSpot's documented default, so we draw none of our own fields inside it and our footer carries only Cancel. It keeps the frame-level error state, because a blocked script renders an empty container with no message of its own. **The border marks a boundary we do not own**: that is why the demo has one and the contact body does not.

### Enterprise Billing page (`/billing-enterprise` → `BillingEnterprise.tsx`, added 2026-09-16)

Own build, not the Pro component. Enterprise is a Support-granted entitlement billed BY SEAT through its own Stripe configuration (H2 PRD §3, §10), so there is no checkout, plan-comparison or cancel dialog. Section order follows the H1 Billing PRD: Plan card (`Enterprise` hero, description, seats subtitle, then the seat facts flat on the card as a hairline-separated `dl` of label/value stat rows, no inset: Seats · Price per seat · Current period · Next invoice (PRD vocabulary: billing unit = seat, people = members; "user" appears only in the live Pro price string "$20 / user / month"); footer sentence + `Contact support` no-op button with the site's `Headset` icon), **Changes this period** table card (Member · Seats +1 or -1 · Date · Amount; removals $0.00), the shared **Credits** and **Payment method** cards (PM approval 2026-09-16; Payment method description "Charged for credit top-ups." since Support handles the seat billing per H2 §10), and **Billing history**, a tabbed card: **Plan** (Date · Description · Seats · Amount · Status, seat charges, `Invoice portal`) and **Balance** (the PAYG ledger: Date · Type · Amount · Balance after, day-grouped gateway messages; `HistoryLedger` takes optional `rows` / `emptyBody`, defaulting to `HISTORY_ROWS` so Pro and Free are unchanged). The Billing history card is the shared `BillingHistorySection` in `src/pages/billing/` with an `Invoice portal` action; rows are billing EVENTS, so there is no per-row download. A billing period is a CALENDAR MONTH anchored to the demo clock's current month (user direction); Pro keeps its anniversary renewal per H1.

Data: `src/data/billing-seats.ts` is the shared seat-billing engine (periods, prorations, additions/removals, `InvoiceRow`, PAYG receipts from `HISTORY_ROWS`); `src/data/billing-enterprise.ts` binds it to calendar months at `ENTERPRISE_SEAT_RATE_USD`, `src/data/billing-pro.ts` binds it to the Pro anniversary (`BILLING_PERIOD_END_DATE`, two periods) at `PRO_SEAT_RATE_USD` and gives Free receipts only. Seat count = accepted members (`MEMBER_ROWS`); pending invitations are not seats. `FORMER_MEMBER_ROWS` are mock departed members so removals and history exist. Three months of history: a monthly seat charge on each 1st billing the people present that day, one prorated row per mid-month addition, no row for removals. The per-seat rate `ENTERPRISE_SEAT_RATE_USD` ($50) is a flagged PLACEHOLDER. `enterpriseBillingView(state)` is the single read the page makes; it carries `creditBalance` / `lastTopUp` / `hasCard` / `ledgerRows` alongside the seat fields, so the shared `CreditsCard` (`balance`, `lastTopUp`) and `PaymentMethodCard` (`empty`) render the zero state from data rather than a page-local branch. `CREDIT_BALANCE_USD` and `lastTopUpLabel()` in `src/data/billing-history.ts` derive both from `HISTORY_ROWS[0].balanceAfter` and the newest `Credits added` row, so the hero ($49.99, rounded in display only) can never disagree with the ledger.

Preview states drive off `?state=` (one-way, not stripped; unknown → `active`) and a `Billing state` Select beside the page title (mock-site affordance like "Viewing as"): `Current` (the org on Enterprise since the first 1st after its owner joined, one month of history before the current one; the Plan tab lists one monthly charge per period plus one prorated row per mid-period join, the Balance tab the shared PAYG ledger), `Upgraded to Enterprise` (day one: the same org, on Pro until today, so credits, card and the PAYG ledger carry over; one prorated first charge, no changes yet), `Billing being set up` (the same converted org, user narrative 2026-09-17: Enterprise granted today, seat billing not yet provisioned, so the seat side is empty: no charge, empty Plan tab, no Changes card, plan card shows deferred values; credits, card and the PAYG Balance ledger carry over from Pro exactly as in every other state. The view still exposes `creditBalance` / `lastTopUp` / `hasCard` / `ledgerRows` and the cards render a zero state when they are empty, so a fresh-org story is one data change away), `Payment failed` (danger `role="alert"` banner with amount + date, newest seat invoice `Failed`). The revoke case renders on `/billing?state=revoked` only. Billing is admin-only on every plan (`nav-sections.ts` drops it for Manager / Member; chrome guard redirects the URL).

Pro and Free plan cards adopted the same Seats / Plan details stat rows on 2026-09-16 (design-lead decision overriding the ticket's "unchanged" line for layout consistency); later that day (user direction) the inset card chrome (`rounded-md border bg-card-muted p-4`) and the `Seats` / `Plan details` sub-title came off all three tiers, so each plan card reads as ONE surface: hero, description and subtitle in a 12px column, then the hairline (`mt-3` + `pt-3`) over the stat rows, the same rhythm the Credits card beside it already used. Pro and Free keep the LIVE product's balance ledger, titled **Billing history** (user: "History" is too vague) (Date · Type · Amount · Balance after, gateway messages grouped per day and expandable), now shared as `src/pages/billing/HistorySection.tsx`; the Enterprise Billing history splits into Plan / Balance tabs (Enterprise only; Pro stays as built). `src/data/billing-pro.ts` (anniversary user charges for Pro) exists but is not rendered on Pro/Free while live shows the ledger. Pro is priced PER SEAT (H2 §3; H1's flat "$X TBD" superseded), 4 seats × $20; the plan-comparison dialog keeps the live strings "$20 per user / month" and "$20/user/month after your 14-day trial ends".
...
- `ManageSubscription.tsx?form=loading|error|done`: one-way preview param for the contact / Book a demo dialog's body (the HubSpot form + scheduler placeholder). Read on every render, never written back and never stripped, the same contract `?state=` has on Billing; absent or unknown → `ready`. **Replaced `BillingFree.tsx?manage=1`** on 2026-09-22: the plan-comparison dialog it opened is now a page, so the sidebar upgrade CTA and the Free Models banner navigate to `/billing-free/plans` / `/billing-default/plans` directly and no param is needed.

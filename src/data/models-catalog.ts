/**
 * GENERATED from the gateway's public catalogue, do not edit by hand.
 *
 * Source: GET https://gateway.constellationgate.ai/v1/models, fetched
 * 2026-09-14 (416 rows). Rows whose id is hand-authored in models.ts are
 * skipped there, so MODELS = curated rows first (prod's popular order), then
 * these in feed order. Field notes:
 *   pricing        the feed advertises the EFFECTIVE billed rate (already
 *                  marked up), so it is stored as list price with markup 1
 *                  on a single pass-through OpenRouter provider row.
 *   providers      the public feed carries no per-account rows; the
 *                  authenticated available-models endpoint does.
 *   name           the feed flattens dots ("Gemini 2 5 Flash"); a digit-dot
 *                  repair and a parameter-size uppercase ("80B") are applied.
 *   description    verbatim from the feed when present, else empty.
 *   releasedAt     from `released` when present, else null (sinks in Newest).
 * Regenerate with the node snippet in change-logs/2026-09/changelog-9-14.md.
 */

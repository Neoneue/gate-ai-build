    ...
    <Popover onOpenChange={handleOpenChange} open={open}>
      ...
      <PopoverContent align="start" className="w-(--anchor-width) p-0">
        ...
        <div className="border-border border-b p-1">
          <button
            className="type-label-14 flex w-full items-center gap-2 rounded-xs px-2 py-1.5 text-left text-foreground outline-none transition-colors duration-150 ease-out hover:bg-accent focus-visible:bg-accent motion-reduce:transition-none"
            onClick={toggleAll}
            type="button"
          >
        ...
        <div className="max-h-56 overflow-y-auto overscroll-contain p-1">
          {filteredOptions.length === 0 ? (
            ...
            filteredOptions.map((option) => (
              <label
                className="type-label-14 flex w-full cursor-pointer items-center gap-2 rounded-xs px-2 py-1.5 text-foreground transition-colors duration-150 ease-out hover:bg-accent has-focus-visible:bg-accent motion-reduce:transition-none"
                key={option.value}
              >
                <Checkbox

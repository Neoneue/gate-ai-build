    ...
    >
      ...
      {variant === "default" ? (
        <TabsPrimitive.Indicator
          ...
          className="absolute z-0 left-(--active-tab-left) top-(--active-tab-top) w-(--active-tab-width) h-(--active-tab-height) rounded-xs bg-card shadow-sm transition-[left,width,top,height] duration-200 ease-out motion-reduce:transition-none"
        />
      ) : null}
      {variant === "line" ? (
        <TabsPrimitive.Indicator
          data-slot="tabs-indicator"
          className="absolute z-0 left-(--active-tab-left) w-(--active-tab-width) bottom-[-1px] h-0.5 bg-neutral-900 transition-[left,width] duration-200 ease-out motion-reduce:transition-none"
        />
      ) : null}
      {children}
    </TabsPrimitive.List>
  ...
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        ...
        "relative z-10 inline-flex h-[calc(100%-1px)] items-center justify-center gap-2 rounded-xs border border-transparent text-sm whitespace-nowrap will-change-transform transition-[colors,scale] duration-150 ease-out active:scale-[0.99] motion-reduce:transition-none motion-reduce:active:scale-100 group-data-vertical/tabs:w-full group-data-vertical/tabs:justify-start focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-1 focus-visible:outline-ring disabled:pointer-events-none disabled:opacity-50 has-data-[icon=inline-end]:pr-1 has-data-[icon=inline-start]:pl-1 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        ...
        "group-data-[variant=line]/tabs-list:px-4 group-data-[variant=line]/tabs-list:pt-2 group-data-[variant=line]/tabs-list:pb-3 group-data-[variant=line]/tabs-list:rounded-none group-data-[variant=line]/tabs-list:bg-transparent group-data-[variant=line]/tabs-list:text-neutral-600 group-data-[variant=line]/tabs-list:hover:bg-neutral-100 group-data-[variant=line]/tabs-list:hover:text-neutral-900 group-data-[variant=line]/tabs-list:data-active:bg-transparent group-data-[variant=line]/tabs-list:data-active:hover:bg-transparent group-data-[variant=line]/tabs-list:data-active:text-neutral-900 group-data-[variant=line]/tabs-list:data-active:font-medium",

...
function ModelPicker({
  ...
  return (
    <Popover onOpenChange={onOpenChange} open={open}>
      {/* Trigger goes from full-bleed to a fixed 288px once the column has
          room. Container query, not viewport: this scaffold renders inside the
          chrome's `<main>` (`@container`), and `@xl` (576px inline-size) is
      ...
      <PopoverContent align="start" className="w-72 p-0" sideOffset={6}>
        ...
        <div className="max-h-64 overflow-y-auto p-1">
          {filtered.length === 0 ? (
            ...
            filtered.map((m) => (
              <button
                className="type-label-14 flex h-8 w-full cursor-pointer items-center gap-2 rounded-xs px-3 text-foreground outline-none hover:bg-accent-muted focus-visible:bg-accent-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset data-[active=true]:bg-accent"

...
colors:
  ...
  chart-4: "oklch(0.70 0.18 290)"  # violet
...
components:
  ...
  input:
    ...
    rounded: "{rounded.sm}"
    height: 32
    padding: "0 12"  # px-3 (all sizes); focus = border-ring + ring-2 ring-ring ring-offset-2; disabled = bg-neutral-100 text-neutral-500
  ...
  card:
    ...
    padding: 16
    elevation: "border + shadow-xs"  # Migrated 2026-05-15 from `shadow-(--shadow-border)` to `border border-border shadow-xs` — the explicit 1px border carries the edge, shadow-xs adds subtle lift. The `--shadow-border` token no longer exists in `index.css`, and the last code comments naming it were rewritten 2026-09-14 (`grep shadow-border src` returns 0). CardFooter: white, no border, no wash (mirrors DialogFooter).
...
Slots: `chart-1` blue · `chart-2` orange · `chart-3` green · `chart-4` violet · `chart-5` coral · `chart-6` teal · `chart-7` amber · `chart-8` magenta. ← code-direct: `src/index.css:205–212`
...
| `hover:bg-neutral-50` / `-100` | `hover:bg-accent-muted` |
...
**Plan-tier families — blue (Pro) and violet (Enterprise) *(added 2026-09-16, user direction; Pro was indigo for a few hours the same day and went back to blue, indigo read off-brand next to the blue nav)*.** Pro sits on the brand `blue-*` ramp at the same rungs `info` uses; Enterprise on Tailwind v4's default `violet-*` scale, which `index.css` does not declare. The chart palette (`--chart-1..8`) is off-limits for anything but chart series. A plan tier is **not a status**, so it never borrows `success` / `warning` / `danger`, and the Pro variant is distinct from `info` by role name, not hue — each tier gets a role-named variant. Three surfaces per tier, and no others:

| Tier | Badge | Plan title (`HeroNumeric`) | Card `tone` edge |
| --- | --- | --- | --- |
| **Pro** | `variant="pro"` → `bg-blue-100 text-blue-700` · `dark:bg-blue-500/15 dark:text-blue-300` | `text-blue-700 dark:text-blue-300` | `tone="pro"` → `border-blue-200 dark:border-blue-500/30` |
| **Enterprise** | `variant="enterprise"` → `bg-violet-100 text-violet-700` · `dark:bg-violet-500/15 dark:text-violet-300` | `text-violet-700 dark:text-violet-300` | `tone="enterprise"` → `border-violet-200 dark:border-violet-500/30` |
| **Free / Default** | `variant="neutral"` (grey) | default `text-foreground`, no colour class | `tone="default"` → `border-border` |

The border is light `-200` over the dark-500-at-30% rung the Callout and the status banners already use, so no new alpha rung enters the system (light stepped to `-200` on 2026-09-16, user direction: `-300` read too heavy). Consumers: the workspace switcher (trigger badge, menu item, compact `ENT.`) and the "Your plan" card on the matching Billing page. **Measured 2026-09-16**, WCAG against the real rendered background, light / dark: Pro badge 6.57:1 / 7.71:1, Pro title 8.09:1 / 8.92:1, Enterprise badge 6.15:1 / 8.35:1, Enterprise title 7.30:1 / 9.66:1, Free badge 7.17:1 / 5.86:1. Every pairing clears 4.5:1, so the titles pass as normal text and not merely as large text. A fourth use of either family needs a line here first.
...
Animate intent, not decoration. Transition only `color` / `background-color` / `border-color`, `box-shadow`, `opacity`, `scale`, and `transform`; never `transition-all`. In an arbitrary list spell the real CSS properties: `colors` is a Tailwind shorthand, not a property, and Tailwind v4 emits `scale-*` as the standalone `scale` property, so a list naming `transform` does not animate a press (both fixed 2026-09-14 on Badge and Card). Reduced motion always wins (`motion-reduce:transition-none` / `motion-reduce:animate-none`).
...
- **`ghost-to-outline` — the one responsive variant** (added 2026-09-09). `ghost` below `lg`, `outline` from `lg`: `hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground lg:border-border lg:bg-card lg:shadow-xs lg:dark:border-input max-lg:dark:hover:bg-muted/50`. The `lg`+ half is byte-for-byte the `outline` recipe (the ghost-only dark hover is fenced behind `max-lg:`), so a control that takes it is **provably unchanged on desktop**. Base already reserves the 1px (`border border-transparent`), so dropping the edge below `lg` moves no pixels. **Consumer:** the top bar's notifications bell — below `lg` the bar is trimmed to four items (logomark, bell, icon-only Ask AI, hamburger) and the hamburger is the only bordered control, so the bell goes borderless with it. The Ask AI twin next to it is a separate `lg:hidden` button and takes plain `ghost`.
  - **Why the breakpoint is inside the recipe**, same argument as `icon-action` above: `variant` is a prop and cannot carry a breakpoint, and re-declaring a variant's border / fill / shadow in a call-site `className` is hand-rolling ([`.claude/rules/no-handrolling.md`](./.claude/rules/no-handrolling.md)). It is deliberately the ONLY responsive entry in the variant list; a second one needs the same justification, in writing, here.
...
- **Checkbox** (`checkbox.tsx`) — `size-4 rounded-[4px]`, `border-input` unchecked / `bg-primary` checked. Hit-target via `after:-inset-x-3 after:-inset-y-2`. Disabled treatment is `data-disabled:cursor-not-allowed data-disabled:opacity-50` (2026-08-25). **Indeterminate is a third painted state (added 2026-08-25):** pass Base UI's `indeterminate` prop and the box paints the SAME `--primary` fill as checked, swapping the `CheckIcon` for a `MinusIcon` — so mixed and checked read as one family and only the glyph carries the difference. It **cannot** ride on `data-checked:`: Base UI suppresses `data-checked` / `data-unchecked` while `data-indeterminate` holds (and sets `aria-checked="mixed"`), so the filled recipe is duplicated onto `data-indeterminate:` and the glyph swap runs off `group/checkbox` on the Root. Both glyphs live inside the one `Indicator` (Base UI renders it for `checked || indeterminate`) with one `hidden` at a time, so `grid place-content-center` sees a single child and the box centres identically in all three states. Consumer: the /notifications Inbox select-all header. **Don't hand-roll a dash** — a some-but-not-all header box that paints empty while announcing "mixed" is the one state a select-all control exists to show. **Generalizable rule:** Base UI renders controls as non-native elements (`<span role="checkbox">`), so the `:disabled` pseudo-class never matches — disabled styling on any Base UI control takes the `data-disabled:` variant (Switch and Select/Menu already do; radio-group.tsx still carries the dead `disabled:` form and needs the same swap the first time anything disables a radio).
...
- **Card** (`card.tsx`) — `rounded-md bg-white border border-border shadow-xs py-4 text-sm text-neutral-900`. **Border + `shadow-xs`, NOT shadow-as-border** (migrated 2026-05-15 from the old `shadow-(--shadow-border)` recipe — see §5). Radius is `rounded-md` (8px) — the card / surface tier (2026-05-10); buttons and chrome stay at `rounded-sm` (6px) so cards read a discrete material step up. **`density` variant** (`default` | `flush`): default = `gap-4 py-4`; flush = `gap-0 py-0` — used whenever a `<Table>` or full-bleed feed (FilterToolbar + Table + TablePaginationFooter stack) lives directly inside the Card and supplies its own internal border-t rhythm. **`tone` variant** (`default` | `danger`, added 2026-08-05): default = `border-border`; danger repoints the edge to **`border-destructive-subtle`** — the 30% rung of the destructive alpha ladder (§2), which derives from `--destructive` and so reads red in both themes on its own (danger-600 light / danger-400 dark). For surfaces whose only action is irreversible. **Deliberately quiet, and the same rung in both themes** — the edge only has to say "this card is dangerous"; at 100% and at 50% it out-shouted the destructive `<Button>` it frames. 30% was verified to still read as danger in light AND dark, so no per-theme step is needed. Use the named rung, never a bare `border-destructive/30`. **Edge only** — fill stays `bg-card`, ink stays neutral, padding and rhythm are untouched, so the card still reads as a peer of the neutral cards beside it and the destructive `<Button>` inside stays the loudest element on it. Consumers: the Settings "Account management" pair (Cancel plan / Delete account and data). Never paint a danger border onto a call site's `className` — the surface tone is the primitive's, like `size` and `density`. **`tone="pro"` and `tone="enterprise"` (added 2026-09-16, user direction)** are the PLAN-TIER edges: `border-blue-200 dark:border-blue-500/30` and `border-violet-200 dark:border-violet-500/30` — the dark side is the 500-at-30% rung the Callout and the status banners use, so no new alpha rung enters the system (light stepped to `-200` on 2026-09-16, user direction: `-300` read too heavy). Like `danger` they are **edge only**: fill, ink, padding and rhythm are untouched, and the tier colour also appears on that page's plan badge and plan title (§2, plan-tier families). Their only consumer is the "Your plan" card on the matching Billing page — `tone="pro"` on `/billing`, `tone="enterprise"` on `/billing-enterprise`. Free and Default keep `tone="default"`. A tier is not a status, which is why these are named by role rather than reusing an `info` blue. **`interactive` variant** (boolean, added 2026-09-14): the whole card is one click target (it navigates or opens a detail), so it takes **the TableRow hover recipe, verbatim** — `hover:bg-accent-muted` + `cursor-pointer` + `transition-[background-color,scale] duration-150 ease-out motion-reduce:transition-none` (the list names `scale`, not `transform`, since 2026-09-14; Tailwind v4 emits the press as the standalone `scale` property). A clickable card and a clickable row must not feel like two systems, so the recipe is copied from `TableRow` rather than invented. **Plain `hover:`, not `hover-fine:`** — the `@custom-variant hover-fine` in `index.css` currently compiles to invalid nested CSS (`(@media …) and (&:hover) { … }`), so every `hover-fine:*` in the codebase is inert; `TableRow` uses plain `hover:`, which is why row hover works at all. If `hover-fine` is ever repaired, this variant moves with it. **Press IS part of it, focus is NOT** (press moved onto the card 2026-09-14): `active:scale-[0.98]` on the card, because `:active` propagates to ancestors and the whole framed card must press as one object; when the scale sat on the inner `RowActionButton` of a flush card only the content shrank while the border stood still. Focus belongs to the interactive child, which owns the real focus ring; a `<div>` cannot take focus, so the card must not pretend to. **On a `density="flush"` card the child fills the card edge to edge and the Card's `overflow-hidden` clips an outer ring, so the child takes `rounded-md focus-visible:ring-inset`** (the `NavTableRow` recipe; fixed 2026-09-14). Consumers: the Models Featured cards and the Free models cards. A `dimmed` Featured card drops `interactive` (no hover, no press) but keeps `cursor-pointer`, because it still navigates. Never paint `hover-fine:bg-accent` onto a call site's `className` — the hover is the primitive's, like `size`, `density`, and `tone`. **`CardHeader` internal gap: `gap-y-1` (4px) / `gap-x-2` (8px), changed from `gap-y-0` on 2026-09-16 (user direction: title and description read as one block).** The 4px is ON the grid and lives in the primitive, so every `CardTitle` / `CardDescription` pair on the site gains it at once — Billing, Payment method, Security, the Teams settings stack, the Activity trend cards, Sign in / Sign up. `gap-x-2` keeps the title column off any `CardAction`. **Still do not pass gap classes at a call site** — the header's rhythm is the primitive's, like `size`, `density` and `tone`; a per-page `gap-y-*` is how two cards start disagreeing. Hand-built sub-card headers that are NOT `CardHeader` (the Billing pages' `Seats` / `Plan details` insets) use `flex flex-col gap-2` — 8px, because a 16px label over 14px copy needs one step more than the 16-over-14 inside a `CardHeader`. Composition: `<Card>` (gap-4 col) → `<CardHeader>` (px-4 grid) → `<CardTitle>` (text-base font-medium leading-snug) → `<CardDescription>` (text-sm/5 text-neutral-500) → `<CardContent>` (px-4) → `<CardFooter>` (p-4, white, **no inner border, no wash** — structural separation comes from spacing alone; the Card's own `border` carries the edge). When a `<CardFooter>` is present, Card auto-applies `pb-0` so the footer hugs the bottom edge. Compact: `data-[size=sm]` → `gap-3 py-3`. **Cross-ref:** mirrors `<DialogFooter>` (short-modal variant) — no inner border, gestalt from spacing. The scroll-modal `<DialogScrollFooter>` is the *other* footer pattern (border-t + tighter `py-4`) — used when content scrolls above the action band.
...
- **SidebarUpgradeCard** (`sidebar-upgrade-card.tsx`, built 2026-08-04 from Figma `sidebar-footer-light` 1255:6256 / `sidebar-footer-dark` 1256:6340) — the "Upgrade to Pro plan" promo pinned beneath the nav in the expanded rail and the mobile nav Sheet. **The one branded surface in an otherwise neutral rail**, and the only consumer of the `--promo-*` family (§2). Recipe: `rounded-md border border-promo-border bg-card p-3 shadow-sm shadow-(color:--promo-shadow) overflow-hidden`, with a full-bleed `.sidebar-upgrade-texture` layer behind the copy, a `type-label-12` title on `text-foreground` over a `gap-1` and a `type-copy-10` line on `text-muted-foreground`, and a 24px `<SparklesIcon>` at `opacity-50` inset 8px from the top right on `text-promo-accent`.
  ...
  **Rest state is the Figma frame exactly; interaction comes from house conventions, not invention.** It is a real `<button>`, so the shared `<SparklesIcon>` picks up its closest-button hover retrofit for free; press is the global `active:scale-[0.98]`; focus is the standard `ring-2 ring-ring ring-offset-2 ring-offset-background`. The texture layer is `aria-hidden` + `pointer-events-none`, and the card sits **outside** the `<nav>` landmark — it is a promo, not a destination.
...
- **Table bulk select** (codified 2026-08-25, `pages/Notifications.tsx` feed) — the Gmail pattern, for tables whose rows have a bulk verb. Three parts, and the constraints are the interesting bit:
  - **Leading checkbox column.** A `w-[6%]` rail (43px at a `min-w-[720px]` table floor — 16px control + the card's 16px gutter) as the FIRST column, ahead of the row identifier. `TableHead` / `TableCell` already zero their right padding when they hold a `role=checkbox` (`[&:has([role=checkbox])]:pr-0`), so the header box and the row boxes land on one x with no call-site padding. Header cell = select-all-on-this-page (`aria-label="Select all on this page"`, `indeterminate` when some-but-not-all — see Checkbox); row cells name their row (`aria-label="Select {thing}: {title}"`). Keep it a plain `<TableHead>`, never `<SortableTableHead>`.
  - **On a clickable row (`NavTableRow`), the checkbox must stop click AND keydown.** The row is a `role="link"` that activates on click and on Enter/Space, so stopping one leaves the other navigating away from the page you were selecting on — the same two-handler shape the per-row `IconActionButton` already uses. Base UI runs the caller's handler BEFORE its own and only skips its own on `preventBaseUIHandler()`, so `stopPropagation()` blocks the row without blocking the toggle.
  - **Selection is PAGE-SCOPED and clears on page change, rows-per-page change, and tab switch.** Select-all means "the rows you can see", never the whole filtered set — a control that silently reached rows on pages the user never opened is the classic bulk-action footgun, and it is unacceptable when the verb has no undo. Hold it as a local `Set` of ids and **derive the count by intersecting with the rendered page**, not from `set.size`: a per-row action can shift the window under a live selection, and the intersection makes the banner self-healing.
  - **The banner is a spanning row inside `<TableBody>`, first child** — `<TableRow className="bg-accent"><TableCell colSpan={N}>`. It PUSHES the data rows down rather than overlaying them or adding a strip of chrome above the card, so the rows it speaks for stay directly beneath it and nothing it covers is half-readable. `bg-accent` is the same fill `TableRow` paints `data-[state=selected]` rows with, so banner + rows read as one selection block, and it needs no `hover:` twin (the base row recipe already hovers to `--accent`). Content: the prose sentence in the cell's inherited copy voice, then the affirmative `Button size="sm"` carrying its lucide glyph on `data-icon="inline-start"`, then a `variant="ghost"` escape. **Spell both grammatical numbers** ("Move 1 notification" / "Move 3 notifications"), never "(s)".
  - **Action columns whose glyph is self-explanatory hide the header LABEL, not the header CELL** — `<span className="sr-only">Actions</span>` inside the `<TableHead>`. Real text in the accessibility tree over an empty `<TableHead aria-label>` (ApiKeys' older form): author-supplied names on empty cells are read inconsistently. `sr-only` is out of flow, so on a `table-fixed` table there is no layout cost.
...
- **Popover** (`popover.tsx`) — Base UI `Popover.*` thin wrapper. Surface: `rounded-sm bg-popover shadow-md` (menu/popover tier). Used by the `<DateRangePicker>` trigger (custom-range surface on the Requests time-scope toolbar), the Mark PIJ event slide (Security page), and `<NotificationsMenu>` (top-bar bell, 2026-08-24). **`positionerClassName` (2026-09-09)** puts classes on Base UI's Positioner, the element it places with an inline `transform`; it exists only to move WHERE a panel sits at a breakpoint. Sole consumer: the bell, which below `lg` is a full-width drop-down hung from the 64px top bar (`max-lg:fixed! max-lg:inset-x-0! max-lg:top-16! max-lg:w-auto! max-lg:transform-none!`, popup `w-full rounded-t-none border-t-0 lg:w-100 lg:rounded-t-sm lg:border-t`, sliding from behind the bar on `--ease-drawer` 300 / 200ms with the Positioner clipping via `max-lg:overflow-hidden`) and at `lg`+ the 400px anchored popover it always was. Visual classes never go on the Positioner. Same dismiss-flicker rule as Dialog: needs `data-closed:fill-mode-forwards` on both popup and overlay, and `onOpenChangeComplete` for URL deep-link cleanup.
...
- **Badge** (`badge.tsx`) — base: `h-5 rounded-xs border border-transparent px-2 font-mono text-xs font-medium tabular-nums uppercase`. `size="xs"` swaps in `h-4 text-2xs` (Micro tier, §3) for the Featured-card tagline only. Transition `transition-[color,background-color,border-color,box-shadow] duration-150 ease-out` (fixed 2026-09-14; it named `colors`, which is not a CSS property, so badge colour never transitioned). **Locked contract (2026-05-11; uppercase + AA contrast 2026-06-04):**
...
- **TextLink** (`text-link.tsx`, codified 2026-05-10) — **inline link affordance, button-by-default.** Renders `<button type="button">` (correct for this codebase's no-router architecture); pass `as="a" href={...}` for a real anchor when navigation is needed. Locked visual recipe: `text-neutral-800 underline decoration-neutral-200 underline-offset-2 hover:decoration-neutral-500 focus-visible:decoration-neutral-500 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-xs`. **No blue** — blue is reserved for info / completed / active-tab / focus. Don't hand-roll the recipe; the className convention block in this doc still exists as the underlying contract, but `<TextLink>` is the single canonical consumer-facing primitive.
- **IconActionButton** (`icon-action-button.tsx`, codified 2026-05-10) — 24px (`size-6`) icon-only button with `after:-inset-2` pseudo-element expanding the hit target to 40×40 without inflating the visual footprint. Recipe: `rounded-xs text-neutral-500 hover:text-neutral-900 hover:bg-accent-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background active:translate-y-px motion-reduce:active:translate-y-0`. `touch-manipulation` kills the 300ms tap delay. **`aria-label` is required** at the type level — icon-only buttons have no accessible name from text content. Extracted from CMP-012's `TopKeysCard` overflow `MoreHorizontal`; any second site would have silently drifted.

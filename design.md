...
**Plan-tier families — indigo (Pro) and violet (Enterprise) *(added 2026-09-16, user direction)*.** Tailwind v4's default `indigo-*` and `violet-*` scales, consumed the same way `info` consumes the default `blue-*` scale; `index.css` declares neither ramp, and the chart palette (`--chart-1..8`) is off-limits for anything but chart series. A plan tier is **not a status**, so it never borrows `info` / `success` / `warning` / `danger` — each tier gets a role-named variant instead. Three surfaces per tier, and no others:

| Tier | Badge | Plan title (`HeroNumeric`) | Card `tone` edge |
| --- | --- | --- | --- |
| **Pro** | `variant="pro"` → `bg-indigo-100 text-indigo-700` · `dark:bg-indigo-500/15 dark:text-indigo-300` | `text-indigo-700 dark:text-indigo-300` | `tone="pro"` → `border-indigo-200 dark:border-indigo-500/30` |

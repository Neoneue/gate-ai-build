...
**Field group label** (codified 2026-05-13 from CMP-018 AutoRechargeDialog) — when a control group sits *under* its label (radiogroup of preset tiles, slider with value display, segmented control) inside a form or dialog, the label is `font-sans text-sm font-medium text-ink-500 m-0` on a `<p>` (or `<FieldLegend variant="label">` if the group is a true `<fieldset>`). **The color is the load-bearing tell:** `text-ink-500` recedes so the values lead the eye — this is what distinguishes a *group label* from `<Label>` (which pairs 1:1 with a single input and stays at the default ink-900 weight). Don't use Eyebrow for this slot — mono UPPERCASE on a group label reads as a chrome strip, not as a form heading; the primitive's own header note explicitly retires it from modal title blocks for the same reason. **Don't promote to ink-900** even when the label feels like it should be "louder" — louder is what `<Label>` already does for paired inputs; a group label by definition is summarizing what's below, so it stays quiet on purpose.

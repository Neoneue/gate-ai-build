...
- **Message style.** Conventional Commits — `feat|fix|refactor|docs|chore(scope): subject` — imperative mood, derived from the actual diff and this session's context. No em dashes.
- **No `Co-Authored-By` trailer.** Commits on this repo carry no co-author attribution. Do not append one, whatever the harness default says.
...
4. Stage the chosen files and commit with a Conventional Commit message (no co-author trailer). Capture the short hash (`git rev-parse --short HEAD`).
...
7. Commit the changelog + index together: `docs(changelog): <summary> (<hash>)`.

...
1. Execute the full `/commit` procedure — read `.claude/commands/commit.md` and follow every step and rule: confirm branch ≠ `main`/`master`, stage intentionally, `npx tsc -b` gate, feature commit with the Co-Authored-By trailer, stamp `change-logs/changelog-<M>-<D>.md`, then the `docs(changelog)` commit.

...
| --- | --- |
| touches visual values (color, type, spacing, radius, tokens), i.e. most UI work | this repo's design contract: `design.md` + `.claude/rules/`. (The bundled `agents/front-end-developer/contract/globals.md` is only a generic fallback for a project that has no contract of its own.) |
| is genuine design or creative work (new component, new layout, visual direction) | `agents/front-end-developer/knowledge/core/craft-methodology.md` — the anti-default methodology (intent-first thinking, craft checks). Not needed for small token/layout edits. |
| writes or edits UI markup | `agents/front-end-developer/skills/web-design-guidelines/SKILL.md` — a11y, semantics, focus, forms, motion, touch targets; apply as you write. |
| needs the agent bundle's own architecture (what loads when, how tokens flow) | `agents/front-end-developer/data-model.md` (6 Mermaid diagrams). Rarely needed for feature work. |
...
2. **Supporting knowledge is EXTRA, used only where `design.md` is silent:** the bundled `agents/front-end-developer/knowledge/`, `contract/globals.md`, the skills, and impeccable inform craft and direction. They NEVER override `design.md`.
...
1. **No `system.md` yet** → Build against **`agents/front-end-developer/contract/globals.md`** + host **`globals.css`** (shadcn semantic tokens). Use **`agents/front-end-developer/knowledge/core/`** (e.g. `craft-methodology.md`, `design-process-rules.md`, `design-recipes.md`, `ux-methodology.md`), **`agents/front-end-developer/knowledge/shadcn/`** (tokens, blocks, theming), **`agents/front-end-developer/skills/web-design-guidelines/SKILL.md`**, and **`agents/front-end-developer/skills/impeccable/SKILL.md`** to establish direction; capture **Theme + Project** in **host `system.md`** when the team is ready (not generic palette dumps).
...
4. **Building** → When `system.md` exists, it wins for what it defines; **`agents/front-end-developer/contract/globals.md`** + **`globals.css`** remain the stable numeric/token floor. Nothing overrides the stack unless the user explicitly opts out.
...
**Principle — quality at creation, not patch-later:** The **`agents/front-end-developer/knowledge/core/`** library exists so you **reason and build well from the first line**—craft, process, UX, motion, full web interface guidelines, pre-ship thinking. Load the right knowledge **early**, with **`web-design-guidelines`**, **`agents/front-end-developer/knowledge/shadcn/`** when implementing shadcn/Tailwind, the **active contract** (`system.md` when present, else **`agents/front-end-developer/contract/globals.md`** + host **`globals.css`**), and **`react-best-practices`** as you implement. **Impeccable slash commands** (`/arrange`, `/typeset`, `/polish`, …) are for **refinement, drift correction, and ship gates**—not a substitute for skipping methodology up front. Defaulting first and “fixing with skills later” wastes time and usually loses craft.
...
| Micro-detail values: concentric radius, optical alignment, surface depth, icon sizing, hit areas, enter / exit and icon transitions | `agents/front-end-developer/skills/better-ui/SKILL.md` (jakubkrehel `better-ui`, installed 2026-09-14) plus its `surfaces.md` / `animations.md` / `icons.md` as the row applies. Exact values, not ranges; design.md tokens still win on any conflict. Settled overrides (2026-09-14, do not re-flag): press `0.98` not `0.96`; lucide stroke a global `1.75`, not keyed to text weight; radius follows the Tailwind ladder 24 / 16 / 8 / 4, not concentric arithmetic; surfaces are `border-border` + `shadow-xs`, never shadow-as-border. |
| Adding a NEW transition to an element that has none (dropdown, modal, panel, number pop-in, icon swap, text swap, success check, skeleton reveal, toast, accordion, streaming text) | `agents/front-end-developer/skills/transitions-dev/SKILL.md` (Jakubantalik `transitions.dev`, installed 2026-09-14). Run its decision rules, load the one numbered recipe file the element matches. Base UI + tw-animate-css stay the mechanism; design.md easings and durations win over the recipe's literals. |
| Tuning motion that ALREADY animates: duration, distance, scale, blur, easing, open/close asymmetry, hover in/out, stagger totals, intent delays | `agents/front-end-developer/skills/transitions-polish/SKILL.md` (add-on to transitions-dev). Match on usage, never nearest number. Its token scale is a reference only; write design.md tokens (`--ease-out`, 100/120/150/200/300ms, press 0.98), never import its `_root.css`. |
...
| **OKLCH** color math — hex/rgb/hsl→oklch conversion, perceptually uniform palette scales (50–950), dark-mode lightness derivation, WCAG/APCA contrast checks, hue-drift detection in HSL palettes, sRGB↔Display P3 gamut clamping, Tailwind v4 `@theme` oklch tokens. Use whenever building or auditing a palette, debugging contrast, or rebinding the host `globals.css` blue ramp | `agents/front-end-developer/skills/oklch-skill/SKILL.md` (+ `color-conversion.md`, `palette-generation.md`, `accessibility-contrast.md`, `gamut-and-tailwind.md`) |
...
| **SVG** graphics, path animation, SMIL, illustrated heroes | `agents/front-end-developer/skills/svg-animations/SKILL.md` — handcrafted SVG + animation; pair `agents/front-end-developer/skills/react-best-practices/` (SVG transform wrapper rule) in React |
...
| Missing design context for Impeccable-style work | `agents/front-end-developer/skills/impeccable/SKILL.md` (teach / craft) |
| Concrete micro-tactics (concentric radius, `text-pretty`, press scale, stagger) | `agents/front-end-developer/knowledge/core/motion-patterns.md` + `the emil-design-eng skill` |
| "Feels off" polish — text-wrap balance/pretty, concentric radii, optical alignment, shadows-over-borders, tabular-nums, font smoothing, icon cross-fade, interruptible transitions | `agents/front-end-developer/skills/make-interfaces-feel-better/SKILL.md` |
| Extract a complete `design.md` (9 canonical sections) from a public URL or from screenshots of an auth-walled site; user points at a page / shares image / pastes URL | `agents/front-end-developer/skills/design-extractor/SKILL.md` |
| **Greenfield project** — scaffold a seeded `design.md` from stack defaults (Tailwind + shadcn) before brand is decided | `agents/front-end-developer/skills/design-seed/SKILL.md` |
| **Push existing React code to Paper for visual iteration** | `agents/front-end-developer/skills/code-to-paper/SKILL.md` |
| **Push existing React code to Figma frame** (team-visible, variable-bound) | `agents/front-end-developer/skills/code-to-figma/SKILL.md` |
| **Bring refined Paper design back to React code** (after canvas iteration) | `agents/front-end-developer/skills/paper-to-code/SKILL.md` |
| **Bring a Figma frame back to React code** — single component or whole flow / screen with multiple states. Enforces verbatim transcription (every text string + icon SVG path traced to a Figma node, never invented). For flows, follows a spec-first pipeline (`references/multi-component-flow.md`) with a persistent type-checked preview script. Honors Code Connect mappings. | `agents/front-end-developer/skills/figma-to-code/SKILL.md` (+ `references/anti-fabrication-examples.md`, `references/multi-component-flow.md`, `references/gotchas.md`) |
| SVG illustration / path / SMIL / CSS-on-SVG animation | `agents/front-end-developer/skills/svg-animations/SKILL.md` |
| React/Next performance (RSC, bundles, lists, memoization) | `agents/front-end-developer/skills/react-best-practices/SKILL.md` |
| **Baseline a11y/UX — methodology while building** (apply rules as you write; optional dedicated review pass) | `agents/front-end-developer/skills/web-design-guidelines/SKILL.md` |
| Compound components, flexible component APIs | `agents/front-end-developer/skills/composition-patterns/SKILL.md` |
| **Product UI** — dashboards, admin, SaaS, tools, data interfaces (not marketing landings); also greenfield direction when **Theme/Project** (`system.md`) is absent or thin | `agents/front-end-developer/knowledge/core/` + `agents/front-end-developer/knowledge/shadcn/` + `agents/front-end-developer/skills/web-design-guidelines/SKILL.md` + `agents/front-end-developer/skills/impeccable/SKILL.md` |
| shadcn/Tailwind/theming implementation | `agents/front-end-developer/knowledge/shadcn/` (`default-tokens.md`, `blocks-and-patterns.md`, `figma-theming.md`, `figma-component-reference.md`) |
| Token architecture, systematic specs | `agents/front-end-developer/contract/globals.md` + `agents/front-end-developer/knowledge/shadcn/default-tokens.md` |
| WCAG + visual review on specific files | `agents/front-end-developer/skills/rams/SKILL.md` |
| Paper + React in parallel from the **active contract** (`system.md` or `agents/front-end-developer/contract/globals.md` + `globals.css`) | `agents/front-end-developer/skills/paper-parallel-build/SKILL.md` |
| Swap global shadcn preset / theme manifest | `agents/front-end-developer/skills/theme-swap/SKILL.md` |
| Brand identity and voice (assets, guidelines) | `agents/front-end-developer/skills/brand/SKILL.md` |

**Stack defaults while implementing (methodology):** **`web-design-guidelines` first** — load at the **start** of UI work and keep applying it while coding (not only before ship). Add **`react-best-practices`** on every non-trivial UI task; add **`composition-patterns`** when designing component APIs. For in-app product UI (not marketing), add **`agents/front-end-developer/knowledge/core/`** + **`agents/front-end-developer/knowledge/shadcn/`** + **`impeccable`** as the craft spine alongside the active contract.
...
2. **While building** — `agents/front-end-developer/skills/web-design-guidelines/SKILL.md` + `agents/front-end-developer/knowledge/core/craft-methodology.md` + `agents/front-end-developer/skills/impeccable/SKILL.md` (and `agents/front-end-developer/skills/impeccable/reference/` as needed) for anti-slop and hierarchy. **`agents/front-end-developer/skills/svg-animations/SKILL.md` whenever the page uses custom SVG** (diagrams, hero art, logos, path motion)—do not rely on generic divs + Tailwind alone for the “designed” moments.
3. **Motion density** — `the emil-design-eng skill` + `agents/front-end-developer/knowledge/core/motion-patterns.md` for baseline; use **`/impeccable animate`**, **`/impeccable delight`**, or **`/impeccable overdrive`** when the brief calls for scroll/hero spectacle (then **`/impeccable optimize`** so it does not ship janky).
4. **Asset / brand** — Route through **`agents/front-end-developer/skills/brand/SKILL.md`** and **`agents/front-end-developer/knowledge/shadcn/`** as needed (icons, banners, token-heavy styling).
5. **Close the loop** — Per milestone: **look at the running page** (screenshot or browser), not only code diff. **`/impeccable critique`** when a major block is done; **`agents/front-end-developer/skills/rams`** on touched files; **`/impeccable polish`** before ship. If the user names a reference site, treat **section count + illustration/motion intent** as a bar to argue with—not optional inspiration.
...
**Typical sequence (new feature — not every step every time):** (1) `shape` if requirements need a brief → (2) build with stack defaults above + **active contract** (`system.md` when present, else `agents/front-end-developer/contract/globals.md` + `globals.css`) → (3) `critique` and/or `audit` when reviewable → (4) `rams` on touched files → (5) targeted row from the table as needed → (6) `polish` when shipping. **If unsure which targeted skill:** `critique` first; it points to other commands.

**Human team shortcut:** `TEAM-UI-WORKFLOW.md` (package root) and `agents/front-end-developer/knowledge/core/skill-workflow.md` (full map + “less is more” redundancy section) align with this table.

### Skill inventory

Counts and descriptions live in **`agents/front-end-developer/knowledge/core/skill-workflow.md`** (full index, redundancy notes, flow diagram). The routing table above is the working contract — use intent, don't scan the folder. A few high-signal reminders:
...
2. **Read the active contract** — **host `system.md`** (Theme + Project) when present; otherwise **`agents/front-end-developer/contract/globals.md`** + host **`globals.css`**
3. **Apply agents/front-end-developer/knowledge/core + web-design-guidelines + agents/front-end-developer/knowledge/shadcn** — product-appropriate layout, UX habits, anti-default patterns (and `agents/front-end-developer/skills/impeccable/reference/` when needed)
...
**Applies to all targets — Figma, Paper, and Code.** Every value must trace to the **contract chain**: **host `system.md`** (Theme + Project) when it exists for that decision; otherwise **`agents/front-end-developer/contract/globals.md`** plus the host app’s **`globals.css`** (and installed shadcn token names). Never invent orphan hex/radius/spacing.
...
**Colors (Paper):** Use Tailwind semantic classes (`bg-primary`, `text-foreground`) when CSS custom properties are configured. When using direct values, use the exact OKLCH values from **`globals.css`** / **`agents/front-end-developer/contract/globals.md`** (or `system.md` when it defines overrides). No guessing hex values.

**Colors (Code):** Use CSS custom properties via Tailwind classes. Never hardcode color values in components.

**Radii:** Figma: bound via `setBoundVariable`. Paper/Code: use Tailwind `rounded-*` per the scale in **`agents/front-end-developer/contract/globals.md`** and **`globals.css`**, or per **`system.md`** when it specifies radii.

**Spacing:** Every padding, itemSpacing, and gap value must come from the spacing scale in **`agents/front-end-developer/contract/globals.md`** / **`globals.css`**, or from **`system.md`** when it tightens/loosens rhythm. Never invent values outside the scale. Same scale applies to Tailwind `p-*`, `gap-*` classes in Paper and Code.

**Typography:** Figma: apply text styles via `setTextStyleIdAsync`. Paper/Code: use Tailwind `text-*`, `font-*` classes matching the type scale in **`agents/front-end-developer/contract/globals.md`** / **`globals.css`** (or `system.md` overrides).
...
- All spacing from the contract chain (`system.md` if it defines rhythm, else `agents/front-end-developer/contract/globals.md` + `globals.css`) — no invented numbers
- Spacing matches project personality when `system.md` says so; otherwise use scale defaults from **`agents/front-end-developer/contract/globals.md`**
...
- Follow `agents/front-end-developer/knowledge/figma/build-recipe.md` for new screens
- Follow `agents/front-end-developer/knowledge/figma/mcp-workflow.md` for MCP tool operations
...
- Run craft checks from `agents/front-end-developer/knowledge/core/craft-checks.md` before presenting to user
...
- **`agents/front-end-developer/knowledge/paper/canvas-building.md`** — HTML patterns, artboard management, Tailwind usage, tables, light/dark (12 sections)
- **`agents/front-end-developer/knowledge/paper/canvas-elements.md`** — icons, images, shaders, modifications
- **`agents/front-end-developer/knowledge/paper/mcp-workflow.md`** — tool selection, read/write patterns (11 sections)
- **`agents/front-end-developer/skills/paper-parallel-build/SKILL.md`** — parallel code + canvas builds
...
- **All values from the contract chain** — spacing/radii/type/colors per `system.md` (when present) or `agents/front-end-developer/contract/globals.md` + host `globals.css`. No invented numbers or hex.

Verification (non-negotiable): after every `write_html` or `update_styles`, call `get_screenshot`, describe what you see, run `agents/front-end-developer/knowledge/core/craft-checks.md`, fix before responding. Paper designs are the dev spec — `get_jsx(format="tailwind")` returns the exact implementation, no translation step.

---

## Motion Defaults (Apply While Building)

Motion is a build-time decision, not a post-build pass. Apply defaults while constructing every interactive element. Canonical reference: **`agents/front-end-developer/knowledge/core/motion-patterns.md`** (easing tokens, duration scale, button/popover/modal/tooltip/stagger patterns, reduced-motion + hover-capability gates, "when not to animate"). Load it before writing any transition or animation code. For deep UI motion, pair **`the emil-design-eng skill`**; for SVG markup motion, **`agents/front-end-developer/skills/svg-animations/SKILL.md`**.
...
1. Check local knowledge first — `ux-methodology.md`, `web-interface-guidelines.md`, `design-process-rules.md`, and `agents/front-end-developer/skills/web-design-guidelines/SKILL.md`
...
Fix failures before showing. Then run `agents/front-end-developer/knowledge/core/pre-ship-quality-checklist.md` as a final sweep.
...
- **`agents/front-end-developer/contract/globals.md`** (inside this agent package) — Layer 1 stable token/layout boilerplate; ships with the agent
...
- `agents/front-end-developer/data-model.md` — agent architecture diagram (how all pieces connect)

### Load just-in-time

**`agents/front-end-developer/knowledge/core/`** — Design methodology (target-agnostic)
...
| Creating or animating **SVG** (illustrations, diagrams, logos, path effects) | `agents/front-end-developer/skills/svg-animations/SKILL.md` — paths, SMIL, CSS, morphing, masks, performance, reduced motion |
...
**`agents/front-end-developer/knowledge/figma/`** — Figma canvas + MCP
...
**`agents/front-end-developer/knowledge/paper/`** — Paper canvas + MCP
...
**`agents/front-end-developer/knowledge/shadcn/`** — shadcn/UI reference (shared by all targets)

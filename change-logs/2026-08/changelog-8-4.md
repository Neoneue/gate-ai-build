...
**`ui/message-block.tsx`** · **`ui/copy-button.tsx`** · **`pages/conversations/ConversationDetail.tsx`** · **`design.md`**

The Messages panel could show you a turn but not hand it to you, and it clipped anything long to a 200px scroll box inside a panel that itself scrolls. Each bubble now carries two controls on the `↳ req_xxxxx` reference line beneath it — copy, then expand, flush right.

- **They live on the footer line, not in the bubble, and that is structural — not a layout preference.** When a message is cross-link selectable the bubble IS a `<button>`, and a button may not contain another button; the footer row is already a sibling. Nesting would have been invalid HTML *and* would have put these clicks inside the element that toggles the trace selection. The corner-overlay version this replaced needed a positioned wrapper, `pointer-events` juggling and a hover-reveal to work around exactly that; on this line none of it is needed.
- **Always visible.** Matching `ask-ai-message.tsx`'s reply action row, which is the app's existing "tools under a message" pattern. No hover reveal, so no opacity transition, no `motion-reduce` gate, and both controls are in the tab order by default rather than needing a `focus-within` escape hatch.
- **`CopyButton` gains `mode="icon" size="icon-action"`**, extracted from that reply row — which had wired the chrome by hand through `useCopyFeedback` and was therefore one paste away from drifting. 32×32 below `lg`, 24×24 from `lg` (SC 2.5.8 AA exactly, with a bigger tap target on touch), glyph 16 → 14px. Success ink is `text-foreground`, **not** the `text-success-600` the other icon sizes use: this row sits in a panel already full of warn/danger color, and quiet chrome should not add a green status report to it. The pair uses the same row's pitch-preserving `gap-0 lg:gap-1`, so the glyphs hold their spacing across the breakpoint.
- **Both halves of the row are independently conditional.** `requestId` is absent on user-input turns and `copyValue` on a turn with no text, so the row renders when *either* exists and `ml-auto` pins the tools right regardless. `min-h-6` keeps a turn that is missing one half aligned with its neighbours.
- **The clipboard gets what the bubble shows.** `messageCopyText()` takes the prose *and* the tool calls the assistant made on that turn, since `messageBody()` renders those as nested `ToolCallCard`s — copying only the prose would have returned less than the user can see. Bodies are typed `ReactNode` but are strings in practice; a `typeof` guard keeps a future JSX body from landing on the clipboard as `[object Object]`. The toast fragment is role-aware: "user message", "assistant message", "tool result".
- **Expand opens a viewer that reformats nothing.** 600 wide, height content-driven between a 240 floor and a 600 ceiling, so a one-word turn settles at 240 rather than padding out dead space. It renders the **same `body` node** the bubble gets — not the plain-text string built for the clipboard — so nested `CALL` cards, `ToolResultCode` and prose keep their own voices. Its only job is removing the `max-h-[200px]` clamp. Verified in the browser: a short turn lands at 600×240, a mid-length one at 600×308, and the longest message in the trace (10,069 chars) fills to 600×600 with the well scrolling 2,912px of content inside a 506px window.
- **The viewer's dismiss glyph is `Minimize2`, the inverse of the row's `Maximize2` — and no primitive was forked to get it.** `DialogContent` hard-codes an `XIcon` but already exposes `showCloseButton`, so the viewer opts out and composes the exported `<DialogClose>` at the same `absolute top-3 right-3` / `icon-sm` / ghost recipe. It remains Base UI's own dismiss: Escape closes, the exit animation is untouched, and the `aria-label` reads "Collapse" so the control is still announced as something that closes the dialog. **That is the route for any future non-X dismiss** — opt out of the slot, don't add a glyph prop to `DialogContent`.
- **The trigger is a real `<DialogTrigger>`, not a controlled `open` boolean.** That is what makes focus-return-to-the-trigger a contract Base UI owns rather than something that happens to work; it also drops the per-message `useState`.
- **The viewer's inner well is required for fidelity, not decoration.** `<ToolCallCard>` is `bg-card` and is specified to invert against the bubble's `bg-card-muted` fill; rendered directly on the modal's own `bg-card` it would be white-on-white and vanish. The well reproduces the bubble's surface, border and `p-4` exactly, minus the clamp, and takes the scroll so its border frames a fixed reading window. Radius steps 16px modal → 8px well → 4px call card.
- **Two merge hazards worth knowing.** `max-h-[600px]` would have *replaced* the `max-h-[90vh]` `DialogScrollContent` supplies — same tailwind-merge group — and overflowed short viewports; the ceiling is written `max-h-[min(600px,90vh)]` so one utility carries both limits. And the scroll had to be handed to the well with `overflow-y-hidden` (same `overflow-y` group as the primitive's `overflow-y-auto`) rather than the ambiguous `overflow-hidden`. Both resolutions were verified against `tailwind-merge` directly, not assumed.
- **`MessageHeading` is now shared** by the bubble's header line and the dialog title, so `Assistant` / `Tool · Read` cannot disagree between them. The bubble's reading voice moved to a `MESSAGE_BODY_VOICE` const for the same reason.

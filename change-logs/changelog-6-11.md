...
- Each entry is tagged with its commit (`[abc1234]`) or ``[2a8d377]``. For a
...
- **Seats copy tightened** `[2a8d377]`: "Billed per user — a seat is added
...
- **Page subtitle** `[2a8d377]`: "Plan, credits, and transaction history." →
  **"Manage your plan, track credit usage, and review every gateway transaction."**
  (also applied to `/billing-free`).
- **Payment method card added** `[2a8d377]`: new full-width middle card
...
- **History updates** `[2a8d377]`: added subtitle "Past charges and credit
...
- **Credits hero reconciled** `[2a8d377]`: `$24.98` → **`$24.99238`** to match
...
- **Credits zeroed** `[2a8d377]`: hero `$24.98` → `$0.00`; "Used this month"
...
- **Card footers → 8px vertical padding** `[2a8d377]`: `CardFooter` on the
  Billing/BillingFree Plan, Credits, and Payment-method cards gained `py-2`
  (8px top/bottom, 16px sides preserved).
- **Buttons → 32px (`size="sm"`)** `[2a8d377]`: all page buttons (Manage
...
- **Button icons** `[2a8d377]`: `Manage subscription` → `ArrowUpCircle`,
...
- **Button variants** `[2a8d377]`: all card/dialog CTAs are outline + 32px,
  ...
  outline.
- **VISA/CARD chip** `[2a8d377]`: bordered chip, **40px** height (`h-10`), 8px
...
- **`/billing-free` Payment method card** `[2a8d377]`: added as a standalone
...
- **`/billing-free` History → EmptyState** `[2a8d377]`: replaced the whole
...
- Price `$30` → **`$29`** (+ caption) to match the Pro page `[2a8d377]`.
- Featured-card outline `border-blue-600/30 ring-blue-600/20` →
  **`border-primary/30 ring-primary/20`** `[2a8d377]`.
- `DialogContent` padding `p-4` → **`p-6`** (24px) `[2a8d377]`.
